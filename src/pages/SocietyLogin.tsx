      ...
      if (isRegisterMode) {
        ...
        if (error) {
          ...
          toast({
            ...
            description: "Your society account has been created! Please check your email to verify your account.",
          });
        ...
        if (error) {
          ...
          // Check if user is in society table
          const { data: society } = await supabase.from('society').select('id').eq('user_id', userId).single();
          if (!society) {
            toast({
              variant: "destructive",
              title: "Login Failed",
              description: "This account is not a society account.",
            });
            setIsLoading(false);
            return;
          }
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-black via-gray-950 to-black px-2 sm:px-4 py-6 sm:py-12 animate-fade-in relative overflow-hidden pulse-pattern">
          ...
          <div className="w-full max-w-sm">
            ...
            <div className="text-center mb-6 sm:mb-8">
              <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">
                {isRegisterMode ? "Society Registration" : "Society Login"}
              </h1>
              <p className="text-pink-200 text-sm sm:text-base">
                {isRegisterMode ? "Register your society" : "Access your society dashboard"}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 sm:py-4 bg-gray-800/50 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 transition-all duration-300 backdrop-blur-sm pr-12"
                    required
                  />
                  <button
                    type="button"
                  ...
                  >
                    {showPassword ? (
                      <EyeOff className="h-4 w-4 text-gray-400 hover:text-pink-400" />
                    ) : (
                      <Eye className="h-4 w-4 text-gray-400 hover:text-pink-400" />
                    )}
                  </button>
                </div>
              </div>
              ...
              >
                {isLoading
                  ...
                  : (isRegisterMode ? "Register Society" : "Sign In")
                }
              </button>
      ...
      <div className="absolute left-0 top-0 w-64 h-full pointer-events-none hidden xl:block">
        ...
        >
          ...
          <p className="text-white text-sm font-medium">Events hosted</p>
          <p className="text-gray-300 text-xs">Monthly activities</p>
        </div>
        ...
        >
          <div className="flex items-center mb-2">
            ...
            <span className="text-xl font-bold text-purple-400">Free</span>
          </div>
          <p className="text-white text-sm font-medium">Event promotion</p>
      ...
      <div className="absolute right-0 top-0 w-64 h-full pointer-events-none hidden xl:block">
        ...
        >
          <div className="flex items-center mb-2">
            <div className="w-3 h-3 bg-cyan-400 rounded-full mr-3"></div>
            <span className="text-2xl font-bold text-cyan-400">Real-time</span>
          </div>
          <p className="text-white text-sm font-medium">Event analytics</p>
          <p className="text-gray-300 text-xs">Track engagement</p>
        </div>
        ...
        >
          <div className="flex items-center mb-2">
            <div className="w-3 h-3 bg-pink-400 rounded-full mr-3"></div>
            <span className="text-2xl font-bold text-pink-400">Simple</span>
          </div>
          <p className="text-white text-sm font-medium">Event creation</p>
          <p className="text-gray-300 text-xs">Intuitive tools</p>
        </div>
        ...
        >
          ...
          <p className="text-white text-sm font-medium">Dashboard access</p>
          <p className="text-gray-300 text-xs">Manage anytime</p>
        </div>
      </div>
    </div>
  );
