...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const eventToInsert: any = { id, created_at, name, description, start_time: parsedStart, end_time: parsedEnd, location, category, society_id };
    if (typeof signup_link === 'string') {
      eventToInsert.signup_link = signup_link;
    }
    const { error } = await supabase.from(tableName).insert([eventToInsert]);
    if (error) throw new Error(error.message);
